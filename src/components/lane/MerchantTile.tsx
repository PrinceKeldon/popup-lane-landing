...
export default function MerchantTile({
  ...
  const firstProduct = merchant.merchant_products?.find(p => p.is_featured) || merchant.merchant_products?.find(p => {
    const imgs = getProductImages(p);
    return imgs.length > 0;
  }) || merchant.merchant_products?.[0];
  const images = firstProduct ? getProductImages(firstProduct) : [];
  return <div className="group relative bg-card border rounded-lg overflow-hidden shadow-[var(--shadow-card)] hover:shadow-[var(--shadow-hover)] transition-all duration-300 hover:-translate-y-1 flex flex-col min-h-[380px]">
      {/* Product Image Carousel */}
      <div className="cursor-pointer" onClick={handleClick}>
        <ProductImageCarousel images={images} brandName={merchant.brand_name} discountBadge={firstProduct?.discount_percentage ? <div className="absolute top-3 left-3 bg-white text-red-600 px-3 py-1.5 text-xs font-bold uppercase tracking-wide shadow-sm z-20">
                {firstProduct.discount_percentage}% OFF
              </div> : firstProduct?.price ? <div className="absolute top-3 left-3 bg-white text-foreground px-3 py-1.5 text-xs font-bold uppercase tracking-wide shadow-sm z-20">
      ...
      <div className="px-4 pb-3 pt-1 space-y-1 flex-1 flex flex-col justify-end relative z-10 rounded-none bg-[#000900]/0">
        ...
        {firstProduct && <>
            {firstProduct.discount_percentage && firstProduct.original_price ? <div className="flex items-baseline gap-2 text-sm bg-[#fbfcfb]/90 rounded px-2 py-1 w-fit">
              ...
              </div> : firstProduct.price ? <div className="flex items-baseline gap-2 text-sm bg-[#fbfcfb]/90 rounded px-2 py-1 w-fit">
        ...
        <div className="flex gap-2 pt-2">
          ...
          <Button size="sm" variant={isSaved ? "default" : "outline"} onClick={e => {
          e.stopPropagation();
          onSave();
        }} className={isSaved ? "bg-foreground hover:bg-foreground/90 text-background transition-all duration-300" : "hover:bg-muted transition-all duration-300"}>
